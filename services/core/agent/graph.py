"""
LangGraph StateGraph —— 企业智行 · Agent 内核编排器（v3）
...
from core.agent.intent import IntentRouter, Intent
from core.agent.preflight import PreflightChecker
from core.agent.registry import ToolRegistry
from core.agent.handlers import TRAVEL_ASSISTANT_SYSTEM_PROMPT
...
class JourneyHubGraph:
    ...
    def _route_intent(self, state: AgentState) -> AgentState:
        """意图分类节点"""
        with node_duration.labels(service="core", node="route").time():
    ...
    def _preflight_check(self, state: AgentState) -> AgentState:
        """规划前置校验：检查关键信息是否齐全"""
        with node_duration.labels(service="core", node="preflight").time():
            user_input = state.get("user_input", "")
            ctx = TraceContext(session_id=state.get("session_id", ""))
    ...
    def _execute_plan(self, state: AgentState) -> AgentState:
        """行程规划工具调用节点"""
        with node_duration.labels(service="core", node="plan").time():
            user_input = state.get("user_input", "")
            ctx = TraceContext(session_id=state.get("session_id", ""))
    ...
    def _execute_chat(self, state: AgentState) -> AgentState:
        """智能问答工具调用节点"""
        with node_duration.labels(service="core", node="chat").time():
            user_input = state.get("user_input", "")
            ctx = TraceContext(session_id=state.get("session_id", ""))
    ...
    def _execute_manage(self, state: AgentState) -> AgentState:
        """行程管理工具调用节点"""
        with node_duration.labels(service="core", node="manage").time():
            user_input = state.get("user_input", "")
            ctx = TraceContext(session_id=state.get("session_id", ""))
    ...
    def _execute_emergency(self, state: AgentState) -> AgentState:
        """应急协助工具调用节点"""
        with node_duration.labels(service="core", node="emergency").time():
            user_input = state.get("user_input", "")
            ctx = TraceContext(session_id=state.get("session_id", ""))
    ...
    def _generate_response(self, state: AgentState) -> AgentState:
        """生成最终回复"""
        with node_duration.labels(service="core", node="respond").time():
            ctx = TraceContext(session_id=state.get("session_id", ""))
    ...
    def invoke(self, session_id: str, user_input: str, model: str = None) -> Dict[str, Any]:
        ...
        """
        with agent_request_duration.labels(service="core", intent="sync").time():
