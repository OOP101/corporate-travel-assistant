...
class ItineraryGenerator:
    ...
    def generate_stream(
        ...
        if not self.llm.is_available():
            ...
            # v3 修复：演示降级也要尊重用户显式确认的场景参数（v2 写死 personal，
            # 导致降级模式下 business 行程被误判个人出游、跳过政策与审批链路）
            explicit = explicit or {}
            carry = carry or {}
            scene = explicit.get("scene") or carry.get("scene") or "personal"
            demo["scene"] = scene
            if explicit.get("start_date"):
                demo["start_date"] = explicit["start_date"]
            if explicit.get("days"):
                demo["days"] = explicit["days"]
            self.last_params = {"scene": scene, "destination": demo.get("destination", "")}
...
"""
    ...
    def _extract_params(self, query: str, preferences: dict, explicit: dict = None, carry: dict = None):
        ...
        if explicit and all(explicit.get(k) for k in REQUIRED_PARAMS):
            ...
            # 命中不全才回退大模型（复杂句式/相对日期仍可靠）。
            # v3 修复：规则部分命中不再整包丢弃——LLM 结果优先、规则补位，
            # 避免「用户说了目的地，澄清时还被重问一遍」。
            ...
            if fast_covers_required(fast):
                ...
                if fast:
                    ...
                    for k, v in fast.items():
                        if v not in (None, "", []) and raw.get(k) in (None, "", [], "null"):
                            raw[k] = v
                            merged += 1
                    if merged:
                        llm_stats["fast_fallback_fields"] = sorted(fast.keys())
