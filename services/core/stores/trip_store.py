...
- 每次 save/update/delete 同步写盘，写盘为原子操作（temp + os.replace）。
...
class TripStore:
    ...
    def _persist(self, trip: dict):
        """将单条行程原子写入 JSON 文件（temp + os.replace，中断不损坏已有数据）。"""
        trip_id = trip.get("trip_id", "unknown")
        file_path = os.path.join(self.data_dir, f"{trip_id}.json")
        atomic_write_json(file_path, trip, logger_=logger, what=f"行程 {trip_id}")
