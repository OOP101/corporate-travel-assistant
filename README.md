...
**77 passed / 0 failed**（v3 基线：内核 API 11 + 审批闭环与鉴权 11 + Agent 层 7 + 内核 Store 原子写 7 + shared 基建 41；2026-09-26 实测）。
覆盖：澄清契约与授权代填死循环回归、确认门禁（未确认绝不审批）、审批状态机、个人场景豁免、
越权访问 404、**审批接口匿名调用 401 / 非审批人 403 / 非管理员列表收敛到本人**、
**手工建单同步置行程待审批**、**行程 / 画像写盘中断不损坏已有数据（原子写 + 并发）**、档案与同行人读写、外接服务检索降级、工具总线容错。
