...
## 本次会话交付：docs 全量实测校准（2026-09-17）
- **任务**："更新 docs 所有文件，不确定的数字进行实测"。两份 PRD 为设计口径文档（S0–S6 / ≤2 轮 / 5 场景与实现一致），核对后无需改动；实测修正集中在 `docs/简历项目经历-企业智行.md`。
- **实测方法**（全部当日跑出，非引用旧记录）：
  - `pytest --basetemp=./.pytest_tmp -q` 全量 → **177 passed / 0 failed**（93.5s），基线保持；
  - 全仓 `APIRouter(` 计数 → planner-core **10 个**（09-15 新增 guides.py，简历"9 个"过时）；
  - 端点装饰器计数 → **82 个**（journey 10 / planner 66 / sense 6）；
  - 解析 bench_models*.json → 首 chunk **3.6~177.4s**——**推翻文档沿用多时的"86~227 秒"**：227 是 mimo 总耗时（wall_total）被混入首 chunk 口径，85.8 被取整为 86；主力 MiMo 系两轮 65~177s；
  - 代码行号定位 → `chat_json` manager.py:**158**（重试循环 :176、预算翻倍 :179，旧记 148 已漂移）；`_signed_params` **47~62**（旧记 47–58）；`_enrich_external` **454**（旧 424）；`_external_prompt_block` **500**（旧 470）；`_attach_external` **537**（旧 507）；
  - 零引用主张复扫 → JWT / Milvus / run_in_executor / to_thread 全仓 **0 命中** ✅；CHROMA_PERSIST_DIR 仅 settings 默认值 + 日志静音 2 处（无库调用）✅；
  - 其余核过成立：APScheduler `IntervalTrigger(minutes=5)` @ engine.py:352（"5 分钟周期"✅）、感知源 5 类 ✅、React `^19.2.8` ✅、Prometheus 三服务接入 ✅、前端 11 页 ✅。
- **文档修正**：简历权威版 Router 9→10、bench 口径改 3.6~177（标注 227 为总耗时）、5 处行号校正、tests"9 文件"→11 文件；第七节新增 **7.1 实测复检表**（指标 × 方法 × 结果 × 处理），核验记录保留旧值作审计痕迹并标注复测日期。
- **不改**：`行程单导出示例.html`（产物示例无数字）、`screenshots/`（重截图需启动服务，铁律禁止主动 start）。
